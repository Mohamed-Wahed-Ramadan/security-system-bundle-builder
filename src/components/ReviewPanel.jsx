...
export default function ReviewPanel({ reviewLines, totals, shipping, guarantee, financingText, onQuantityChange, saveForLater, saveMessage }) {
  return (
    <aside className="review-panel">
      ...
      {GROUP_ORDER.filter((g) => reviewLines[g]?.length).map((group) => (
            ...
            <div className="review-line" key={line.key}>
              ...
              {!line.isPlan && !line.locked ? (
                <QuantityStepper
                  size="sm"
                  value={line.qty}
                  onChange={(next) => onQuantityChange(line.productId, line.variantId, next)}
                />
              ) : (
      ...
      <button type="button" className="btn btn--primary review-panel__checkout" onClick={() => window.alert("This is a prototype — checkout isn't wired up yet!")}>
        Checkout
      </button>
